...
#define CET_TIME_ZONE_DAYLIGHT_SAVINGS_DIFF 1

#define GMT_OFFSET_SECS (CET_TIME_ZONE * 3600)
#define GMT_OFFSET_SECS_DST ((CET_TIME_ZONE + CET_TIME_ZONE_DAYLIGHT_SAVINGS_DIFF) * 3600)
...
static const char* host = SECRET_IOTHUB_FQDN;
static const char* mqtt_broker_uri = "mqtts://" SECRET_IOTHUB_FQDN;
static const char* device_id = SECRET_DEVICE_ID;
static const int mqtt_port = AZ_IOT_DEFAULT_MQTT_CONNECT_PORT;

static const char* pingHost = "timoknapp.com";
...
static AzIoTSasToken sasToken(
    &client,
    AZ_SPAN_FROM_STR(SECRET_DEVICE_KEY),
...
static TemperatureData readTemperature()
{
  ...
  {
    Logger.Error("Failed reading from DHT sensor!");
    return { 0.0, 0.0, 0.0 };
  }

  Logger.Info("Temperature: " + String(temperatureC) + " °C (" + String(temperatureF) + " °F)");
  Logger.Info("Humidity: " + String(humidity) + " %");
...
static void getTelemetryPayload(az_span payload, az_span* out_payload)
{
  ...
  payload = az_span_copy(payload, AZ_SPAN_FROM_STR("{ \"temperatureCelcius\": "));
  (void)az_span_u32toa(payload, tempData.temperatureC, &payload);
  payload = az_span_copy(payload, AZ_SPAN_FROM_STR(", "));
  payload = az_span_copy(payload, AZ_SPAN_FROM_STR("\"temperatureFahrenheit\": "));
  (void)az_span_u32toa(payload, tempData.temperatureF, &payload);
  payload = az_span_copy(payload, AZ_SPAN_FROM_STR(", "));
  payload = az_span_copy(payload, AZ_SPAN_FROM_STR("\"humidity\": "));
  (void)az_span_u32toa(payload, tempData.humidity, &payload);
  payload = az_span_copy(payload, AZ_SPAN_FROM_STR(", "));
